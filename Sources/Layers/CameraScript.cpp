...
namespace DENG {
	...
	void CameraScript::InitializePosition(const TRS::Vector3<float>& _vPosition) {
		m_cameraTransformer.AbsoluteMoveX(_vPosition.first);
		m_cameraTransformer.AbsoluteMoveY(_vPosition.second);
		m_cameraTransformer.AbsoluteMoveZ(_vPosition.third);
	}
}